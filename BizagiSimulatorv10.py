...
def discrete_event_simulation(max_arrival_count, arrival_interval_minutes, max_time, paths, simulation_metrics):
    resource_busy_time = {resource: 0 for resource in simulation_metrics['Resource'].dropna().unique()}
    event_queue = []
    active_tokens = {}
    ...
    while event_queue and current_time <= max_time:
        ...
        if event.event_type == 'start':
            ...
            if resource_name:
                ...
                resource_busy_time[resource_name] += duration
                active_resources[resource_name] -= 1
                logging.info("Resource '%s' released by Token %d.", resource_name, event.token_id)
...
def main():
    ...
    max_time = 120  # Example simulation time limit
